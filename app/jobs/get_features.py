...
class GetFeaturesJob:
    ...
    async def _fetch_features(
        ...
        if response.status_code == 200:
            ...
            await save_to_json(platform_features, file_path)
    ...
    async def _fetch_and_archive_features(self, releases: Dict[str, Any]):
        ...
        try:
            ...
            await save_to_json(self.unique_features, unique_features_file, 4)
    ...
    async def _fetch_online_releases(self, platforms: Dict[str, Any]) -> Dict[str, Any]:
        ...
        await save_to_json(releases, self.config.PROJECT_DATA_DIR / "releases.json", 4)
        return releases
